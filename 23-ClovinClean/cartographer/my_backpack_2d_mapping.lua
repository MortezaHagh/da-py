-- Copyright 2016 The Cartographer Authors
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
-- You may obtain a copy of the License at
--
--      http://www.apache.org/licenses/LICENSE-2.0
--
-- Unless required by applicable law or agreed to in writing, software
-- distributed under the License is distributed on an "AS IS" BASIS,
-- WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
-- See the License for the specific language governing permissions and
-- limitations under the License.

include "map_builder.lua"
include "trajectory_builder.lua"

options = {
  map_builder = MAP_BUILDER,
  trajectory_builder = TRAJECTORY_BUILDER,
  map_frame = "map",
  tracking_frame = "base_link",
  published_frame = "base_link",
  odom_frame = "odom",
  provide_odom_frame = true,
  publish_frame_projected_to_2d = false,
  use_pose_extrapolator = on,
  use_odometry = false,
  use_nav_sat = false,
  use_landmarks = false,
  num_laser_scans = 1,
  num_multi_echo_laser_scans = 0,
  num_subdivisions_per_laser_scan = 1,
  num_point_clouds = 0,
  lookup_transform_timeout_sec = 0.5,
  submap_publish_period_sec = 0.3,
  pose_publish_period_sec = 2e-3,
  trajectory_publish_period_sec = 30e-3,
  rangefinder_sampling_ratio = 1.,
  odometry_sampling_ratio = 1.,
  fixed_frame_pose_sampling_ratio = 1.,
  imu_sampling_ratio = 1.,
  landmarks_sampling_ratio = 1.,
}

num_threads = 16

MAP_BUILDER.use_trajectory_builder_2d = true
-- TRAJECTORY_BUILDER.pure_localization_trimmer = {
--     max_submaps_to_keep = 10,
--  }

TRAJECTORY_BUILDER_2D.min_range = 0.1
TRAJECTORY_BUILDER_2D.max_range = 16.000000
TRAJECTORY_BUILDER_2D.missing_data_ray_length = 5.5 --changed was 5.5
TRAJECTORY_BUILDER_2D.use_imu_data = false
TRAJECTORY_BUILDER_2D.use_online_correlative_scan_matching = true

TRAJECTORY_BUILDER_2D.num_accumulated_range_data = 38.

TRAJECTORY_BUILDER_2D.ceres_scan_matcher.ceres_solver_options.num_threads = num_threads
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.ceres_solver_options.use_nonmonotonic_steps = true
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_range = 15. -- Decrease
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_length = 0.5 -- Increase
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.min_num_points = 10
--TRAJECTORY_BUILDER_2D.loop_closure_adaptive_voxel_filter.min_num_points = 10 -- Decrease
--TRAJECTORY_BUILDER_2D.loop_closure_adaptive_voxel_filter.max_range = 20. -- Decrease
--TRAJECTORY_BUILDER_2D.loop_closure_adaptive_voxel_filter.max_length = 3 -- Increase
TRAJECTORY_BUILDER_2D.voxel_filter_size = 0.05 -- Increase
TRAJECTORY_BUILDER_2D.submaps.grid_options_2d.resolution=0.05 -- Increase
-- TRAJECTORY_BUILDER_2D.submaps.range_data_inserter.probability_grid_range_data_inserter.hit_probability = 0.51
-- TRAJECTORY_BUILDER_2D.submaps.range_data_inserter.probability_grid_range_data_inserter.miss_probability = 0.49
--LOCALIZACJA--
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 60.000000
TRAJECTORY_BUILDER_2D.motion_filter.max_time_seconds = 1
TRAJECTORY_BUILDER_2D.motion_filter.max_angle_radians = math.rad(9)
TRAJECTORY_BUILDER_2D.motion_filter.max_distance_meters = 0.05
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 10
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 40
--added
--TRAJECTORY_BUILDER_2D.ceres_scan_matcher.occupied_space_weight = 10.
--TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 10. 
--TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 40.

--TRAJECTORY_BUILDER_2D.motion_filter.max_time_seconds = 0.1

--TRAJECTORY_BUILDER_2D.imu_gravity_time_constant = 9.8123
--POSE_GRAPH.optimization_problem.huber_scale = 1e2
--added end

--TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.linear_search_window = 0.1
--TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.angular_search_window = math.rad(15.)
--TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.translation_delta_cost_weight = 0.1
--TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.rotation_delta_cost_weight = 1e-1

--TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 2e2
--TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 4e2



-----------------TUNE THESE PARAMETERS FOR LOW LATENCY-------------------------------

------------Global SLAM------------
-- POSE_GRAPH.optimize_every_n_nodes = 20 -- Decrease --added was 1
--added
POSE_GRAPH.optimization_problem.odometry_rotation_weight = 10
--POSE_GRAPH.optimization_problem.odometry_translation_weight = 1.
--POSE_GRAPH.optimization_problem.fixed_frame_pose_translation_weight = 0.1
--POSE_GRAPH.optimization_problem.fixed_frame_pose_rotation_weight = 0.1
--added finish
MAP_BUILDER.num_background_threads = num_threads -- Increase up to number of cores
-- POSE_GRAPH.global_sampling_ratio = 0.0012 -- Decrease
POSE_GRAPH.constraint_builder.sampling_ratio = 0.1
POSE_GRAPH.constraint_builder.ceres_scan_matcher.ceres_solver_options.num_threads = num_threads


POSE_GRAPH.optimization_problem.ceres_solver_options.num_threads = num_threads
POSE_GRAPH.optimization_problem.ceres_solver_options.use_nonmonotonic_steps = true
POSE_GRAPH.optimization_problem.ceres_solver_options.max_num_iterations = 30
POSE_GRAPH.constraint_builder.min_score = 0.63
POSE_GRAPH.optimize_every_n_nodes = 120 -- Increase
-- POSE_GRAPH.gloal_constraint_search_after_n_seconds = 0.5 -- Increase
-- TRAJECTORY_BUILDER_2D.ceres_scan_matcher.ceres_solver_options.max_num_iterations = 5 -- Decrease

-- POSE_GRAPH.constraint_builder.global_localization_min_score = 0.55
---------Global/Local SLAM---------
--TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.min_num_points = 100 -- Decrease

--POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher.linear_search_window = 0.5
--POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher.angular_search_window = math.rad(3.)
POSE_GRAPH.constraint_builder.max_constraint_distance = 7.


--TRAJECTORY_BUILDER_2D.min_z = -0.5
--TRAJECTORY_BUILDER_2D.max_z = 0.5

--TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.translation_delta_cost_weight = 1e-3
--TRAJECTORY_BUILDER_2D.ceres_scan_matcher.occupied_space_weight = 40

--    linear_search_window = 0.1,
--    angular_search_window = math.rad(20.),
--    translation_delta_cost_weight = 1e-1,
--    rotation_delta_cost_weight = 1e-1,

--  ceres_scan_matcher = {
--    occupied_space_weight = 1.,
--    translation_weight = 10.,
--    rotation_weight = 40.,


return options



